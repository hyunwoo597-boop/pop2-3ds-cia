...
int main(int argc, char* argv[]) {
	...
	try {
		...
		/* Bundled-game launcher: skip the generic DOSBox splash. */

#if defined(CTR_ROMFS)
		if (R_FAILED(romfsInit())) {
			ctr_sys_error(true, "Unable to open bundled game data (RomFS).");
		}

		if (!ctr_install_bundled_game()) {
			ctr_sys_error(true, "Unable to install Prince of Persia 2 to sdmc:/3ds/POP2/. Check free space and SD-card access.");
		}
#endif
		...
		/* This build is a dedicated POP2 launcher. Always use the bundled
		 * configuration and never show the generic configuration picker. */
#if defined(CTR_ROMFS)
		control->ParseConfigFile("romfs:/dosbox-pop2.conf");
#endif

		if(!control->configfiles.size()) {
			ctr_sys_error(true, "Bundled DOSBox configuration could not be loaded.");
		}
...
#if defined(CTR_ROMFS)
		if (!ctr_bundled_game_ready()) {
			ctr_sys_error(true, "Prince of Persia 2 installation is incomplete.");
		}
		MountDOSBoxDir('C', CTR_BUNDLED_GAME_SD_DIR);
#endif
